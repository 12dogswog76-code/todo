/**
 * pryd.js v4.5 — забирает с prydwen.gg тир-лист и данные по сборкам агентов
 ...
 *   goalNotes  — примечания к целям из скобок: «72% Grants full 30% team buff»
 ...
 *
 * v4.5 — разбор целевых значений. Прежняя регулярка теряла данные в трёх
 * случаях: «PEN RATIO: 38% - 72%» (процент после первого числа обрывал
 * диапазон, у Рины оставалось «38%»), «Energy Regen: 2.88» (дробь обрезалась
 * до 2), «...enough.HP: 10500+» (подпись, приклеенная к концу предложения, не
 * опознавалась, и цель пропадала — так у Астры Яо потерялась HP). Плюс
 * восстановление энергии разведено на два ключа: er — итоговое значение из
 * игры, erp — процентный бонус («0 - 130%»); складывать их в один нельзя.
 */

(async () => {
  // Версия печатается первой строкой. Скрипт подтягивают с сайта одной
  ...
  function readAgent(doc, slug) {
    ...
    //
    // Три места, где прежняя регулярка врала:
    //  1. «PEN RATIO: 38% - 72%» — процент стоит и после первого числа. Раньше
    //     после «38» ждали сразу дефис, натыкались на «%» и теряли верхнюю
    //     границу: у Рины цель превращалась в «38%» вместо «38 – 72%».
    //  2. «Energy Regen: 2.88» — дробную часть съедало `[\d,]+`, и у Велины
    //     цель 2.88 становилась 2.
    //  3. Тире бывает и длинным (–, —), а между числами встречается «to».
    // Плюс забираем примечание в скобках: «(72% Grants full 30% team buff at
    // Core Passive 6)» — это часть цели, без неё непонятно, зачем верхняя
    // граница вообще нужна.
    const num = '([\\d,]+(?:\\.\\d+)?)';
    ...
    while ((gm = gre.exec(eg))) {
      // Подпись бывает приклеена к концу предыдущего предложения: у Астры перед
      // целью идёт абзац пояснения, и в разметке получается «...until you have
      // enough.HP: 10500+». Берём то, что после последней точки, — иначе стат
      // не опознаётся и цель молча теряется.
      let k = STAT[gm[1].trim().split('.').pop().trim().toUpperCase()];
      if (!k) continue;
      // Процент может стоять у любого из двух чисел: у Рины «38% - 72%»,
      // у Астры «0 - 130%».
      const pct = gm[3] === '%' || gm[5] === '%';
      // Восстановление энергии на prydwen идёт в двух несовместимых видах:
      // у Велины «2.88» — итоговое значение из игры, у Астры «0 - 130%» и у
      // Орфеи «80%+» — суммарный бонус с дисков. Складывать их в один ключ
      ...
      if (isFinite(lo)) {
        goals[k] = [lo, hi];
        const note = clean(gm[6] || '');
        // «(Optional)» и «(Before Combat)» ничего не объясняют — только шум.
        if (note && note.length > 12 && !/^optional$/i.test(note)) gnotes[k] = note;
      }
    }
    out.goals = goals;
    out.goalNotes = gnotes;
